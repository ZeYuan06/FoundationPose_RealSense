...
from typing import OrderedDict, Tuple, Union, Dict, Any
import queue
import numpy as np
...
class SimpleTrajectoryInterpolator:
    """Simple trajectory interpolator to replace the diffusion policy dependency."""
    
    def __init__(self, times=None, poses=None):
        if times is not None and poses is not None:
            self.current_pose = np.array(poses[0])
            self.target_pose = np.array(poses[0])
            self.start_time = times[0]
            self.target_time = times[0]
        else:
    ...
    def __call__(self, times):
        """Interpolate pose at given times."""
        ...
        for t in times:
            if t <= self.start_time:
                results.append(self.current_pose.copy())
            elif t >= self.target_time:
                results.append(self.target_pose.copy())
            else:
                # Linear interpolation
                alpha = (t - self.start_time) / (self.target_time - self.start_time)
                interpolated = self.current_pose + alpha * (self.target_pose - self.current_pose)
